...
namespace App\Http\Controllers\services\iq\zain\mediaworld;
...
class Gamebase extends Controller
{
    private $config = [
        'cmpid' => 1086,
        'country' => 'iq',
        'partner' => 'zain',            // URL/namespace segment - matches services\iq\zain
        'operator' => 'mediaworld',       // URL/namespace segment - matches services\iq\zain\mediaworld (placeholder, confirm actual operator/aggregator name)
        'operator_display' => 'IQ_zain',          // human-readable carrier name, per campaign sheet
        'offer_name' => 'gamebase',
        'service_name' => 'Gamebase',
        'pin_length' => 5,
    ...
    private function checkAntiFraud($msisdn, $txid, $token, $ip, $ua)
    {
        ...
        try {
            ...
            $resp = Http::timeout(10)->get(
                $this->config['base_url'] . $this->config['anti_fraud_path'],
                $params
            );

            return [
                'passed' => $resp->successful() &&
                    (($resp->json()['response'] ?? null) === 'SUCCESS'),
